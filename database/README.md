...
-   POST
    _ /getCustomerStatus
    _ Consulta si el usuario identificado por un dado RFID está aprobado o no. Debido a que se deben mandar datos se usa el método POST, no GET.
    _ Inputs:
    _ `rfid`: RFID de un usuario. Integer.
    _ `mac_address`: MAC address del NodeMCU. Este identificador se usa para determinar el autobús. Integer.
    _ Outputs:
    _ `customer_status`: "APPROVED", "UNAPPROVED", o "UNIDENTIFIED", el último en caso de que el RFID no se encontró en la base de datos (y por ende no tiene acceso). String.
    /_ Algunos detalles del endpoint están por decidirse. \*/
